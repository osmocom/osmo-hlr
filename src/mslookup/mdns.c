 ...
 * \returns allocated buffer with the resulting domain (i.e. "sip.voice.123.msisdn") on success, NULL on failure.
 */
						   ...
						   uint16_t *packet_id)
{
	...
	OSMO_ASSERT(query);
	if (osmo_mslookup_query_init_from_domain_str(query, req->domain) < 0)
		goto error;
			    ...
			    struct osmo_mslookup_query *query, struct osmo_mslookup_result *result)
{
	...
	if (osmo_mslookup_query_init_from_domain_str(query, ans->domain) < 0)
		goto exit_free;
